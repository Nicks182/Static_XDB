...
namespace Services
{
    ...
    {
        private void _Push_To_Files_Synonyms()
        {
            if (G_DoCancel == true)
            {
                return;
            }
            Srv_SMO_Type L_Type = Srv_SMO_Type.Syn;
            ...
            var L_List = _Get_Synonyms();// G_Database.Synonyms.Cast<Synonym>().ToList();
            ...
            foreach (var L_Object in L_List)
            {
                ...
                G_Srv_Progress._SetProgress(6, "Pushing Synonyms: " + L_Object.Name);
